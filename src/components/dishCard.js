...
function DishCard(props){
    return (
        <Container className="DishCard flex-row">
            <Link to={props.link}><img src={props.img} alt='dish' /></Link>
            <div className="preview flex-column">
                <h1>{props.name}</h1>
                <div className="line flex-row">
                  <p style={{display: `${props.displayCategory}`}}>Category: {props.category}</p>
                  <p style={{display: `${props.displayArea}`}}>Area: {props.area}</p>
                </div>
            </div>
        </Container>
    )
